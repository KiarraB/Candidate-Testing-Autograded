...
function askQuestion() {
  ...
  for (i = 0;i < questions.length; i++) {
    candidateAnswers.push(input.question(`${questionNum[i]} ${questions[i]}`));
...
function gradeQuiz(candidateAnswers) {
  ...
  if (grade < 80) {
    console.log(`>>> Overall Grade: ${grade}% (${candidateCorrectAnswers.length} out of 5 responses correct) <<<
>>> Status: FAILED <<<`);
  } else {
    console.log(`>>> Overall Grade: ${grade}% (${candidateCorrectAnswers.length} out of 5 responses correct) <<<
>>> Status: PASSED <<<`);
